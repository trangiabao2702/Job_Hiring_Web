...
class EmployerController {
    ...
    //[POST] // edit_profile_employer
    async post_edit_profile_employer(req, res, next) {
        try {
            if (req.isAuthenticated()) {
                const dataUpdate = req.body;
                // get information of employer
                const _employer = req.session.passport.user;
                const updateEmployer = await employerModel.updateInfoEmployer(_employer.id, dataUpdate, req.file);
                if (updateEmployer) {
                    req.flash("message", "Cập nhật thông tin công ty thành công");
                    //res.end(); // important to update session
                } else {
                    req.flash("messageDanger", "Cập nhật thông tin công ty không thành công !");
                }
                const user = await employerModel.getEmployerByEmail(_employer.email);
                req.session.passport.user = user;
                res.status(200).redirect("/employer/edit_profile_employer");
            } else {
                res.redirect("/auth/sign_in");
            }
        } catch (error) {
